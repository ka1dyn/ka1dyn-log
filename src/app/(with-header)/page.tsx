...
export default function Page() {
  ...
  const recommendedSlugs = slugs.filter(
  ...
  return (
    <div className="flex flex-col gap-20">
      ...
      {/* ── 1. Hero ── */}
      <section className="relative flex flex-col md:flex-row items-center justify-between gap-10 rounded-2xl bg-primary/5 border border-primary/10 px-10 py-14 overflow-hidden">
        {/* 좌측 텍스트 */}
        <div className="flex flex-col gap-6 z-10">
          <div className="flex flex-col gap-3">
            <h1 className="text-4xl font-bold tracking-tight">Dev Library</h1>
          ...
          <Link
            href="/blog/published"
            className="w-fit flex gap-1 bg-primary/20 transition-all duration-200 ease-out hover:bg-primary hover:text-primary-foreground border border-primary/20 px-5 py-2.5 rounded-full font-medium"
          >
            글 목록 보기 →
          </Link>
        </div>
        ...
        {/* 우측 장식 */}
        <div className="relative w-72 h-48 shrink-0 opacity-60 pointer-events-none select-none hidden md:block">
          {/* 아이콘 군집 */}
          ...
          {/* 코드 스니펫 블록 */}
          <div className="absolute bottom-0 right-0 bg-primary/10 border border-primary/20 rounded-xl px-5 py-4 font-mono text-xs text-primary/70 leading-relaxed">
      ...
      {/* ── 2. 통계 배지 ── */}
      <div className="flex gap-5 text-sm items-center font-light justify-center md:justify-start">
        <span>
          총 <span className="font-bold">{totalCount}편</span>
        </span>
        <div className="h-4 w-px bg-primary/30" />
        <span>최근 업데이트: {recentDate}</span>
        <div className="h-4 w-px bg-primary/30" />
        <span>
          <span className="font-bold">{seriesCount}</span>개 시리즈
        </span>
      </div>
      ...
      {/* ── 4. 최근 글 ── */}
      <section className="flex flex-col gap-6">
        <div className="flex flex-col items-center md:items-start gap-1 md:flex-row md:justify-between md:items-end">
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 w-fit mx-auto md:mx-0 justify-items-center">
          <HomeCards
